...
if [ "${RECOVER_CONTROL_PLANE_TEST}" != "false" ]; then
  ...
  ansible-playbook ${ANSIBLE_LOG_LEVEL} -e @${CI_TEST_REGISTRY_MIRROR} -e @${CI_TEST_VARS} -e local_release_dir=${PWD}/downloads -e etcd_retries=10 --limit etcd,kube_controller_nodes:!fake_hosts recover-control-plane.yml
fi
